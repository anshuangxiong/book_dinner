...
body {
	...
	background: #ddd url(jsps/image/editpass.jpg) repeat top left;
	font-weight: 300;
	font-size: 15px;
	color: #333;
...
<body>
	<h1 align="center">修改密码第一步</h1>
	<div style="align:center">${msg }</div>

	<section class="main">
	<form class="form-4" target=""
		action="<c:url value='/userServlet?method=updatePass1'/>"
		method="post">
		<h1>修改密码</h1>
		<p>
			<label for="login">Username</label> <input type="text"
				name="userName" placeholder="用户名" required> <label
				id="userNameError">${errors.userNameError }</label>
		</p>
		<p>
			<label>Email</label> <input type="text" name="userEmail"
				placeholder="注册邮箱" required> <label id="userEmailError">${errors.userEmailError }</label>
		</p>
		<p>
			<label for="login">VerifyCode</label> <input type="text"
				name="checkNum" id="checkNum" placeholder="验证码" /> 
				<label
